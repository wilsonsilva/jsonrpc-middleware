...
RSpec.describe JSONRPC::InvalidRequestError do
  describe '#initialize' do
    ...
    context 'with a custom message' do
      let(:error) { described_class.new('Custom invalid request error') }

      it 'uses the given message' do
        expect(error.message).to eq('Custom invalid request error')
      end
    end

    context 'with optional data' do
      let(:error) { described_class.new(data: { 'field' => 'missing jsonrpc' }) }

      it 'sets data' do
        expect(error.data).to eq('field' => 'missing jsonrpc')
      end
    end
    ...
    context 'without data' do
      ...
      it 'returns a JSON string with code and message' do
        expect(error.to_json).to eq(
          '{"code":-32600,"message":"The JSON payload was valid JSON, but not a valid JSON-RPC Request object."}'
        )
      end
    end
  end
  ...
  describe '#==' do
    ...
    context 'when the other object is a base Error with the same code and message' do
      it 'returns false' do
        ...
        expect(error).not_to eq(JSONRPC::Error.new(message, code: -32_600))
      end
    end

    context 'when the data differs' do
      it 'returns false' do
        expect(error).not_to eq(described_class.new(data: { 'field' => 'missing jsonrpc' }))
      end
    end
  ...
  describe '#hash' do
    ...
    context 'when an error and its parent class share to_h' do
      it 'returns different hash codes' do
        ...
        expect(error.hash).not_to eq(JSONRPC::Error.new(message, code: -32_600).hash)
      end
    end
