...
RSpec.describe JSONRPC::InternalError do
  describe '#initialize' do
    context 'with no arguments' do
      let(:error) { described_class.new }

      it 'sets the default message' do
        expect(error.message).to eq('Internal JSON-RPC error.')
      end

      it 'sets code to -32603' do
    ...
    context 'with a custom message' do
      let(:error) { described_class.new('Custom internal error') }

      it 'uses the given message' do
        expect(error.message).to eq('Custom internal error')
      end
    end

    context 'with optional data' do
      let(:error) { described_class.new(data: { 'exception' => 'NoMethodError' }) }

      it 'sets data' do
        expect(error.data).to eq('exception' => 'NoMethodError')
      end
    end
  ...
  describe '#to_h' do
    context 'without data' do
      ...
      it 'returns a hash with code and message' do
        expect(error.to_h).to eq(code: -32_603, message: 'Internal JSON-RPC error.')
      end
    end
  ...
  describe '#to_json' do
    context 'without data' do
      ...
      it 'returns a JSON string with code and message' do
        expect(error.to_json).to eq('{"code":-32603,"message":"Internal JSON-RPC error."}')
      end
    end
  end
    ...
    context 'without request_id' do
      ...
      it 'returns a response hash with nil id' do
        expect(error.to_response).to eq(
          jsonrpc: '2.0',
          error: { code: -32_603, message: 'Internal JSON-RPC error.' },
          id: nil
        )
      end
    end
  ...
  describe '#==' do
    ...
    context 'when the other object is a base Error with the same code and message' do
      it 'returns false' do
        expect(error).not_to eq(JSONRPC::Error.new('Internal JSON-RPC error.', code: -32_603))
      end
    end

    context 'when the data differs' do
      it 'returns false' do
        expect(error).not_to eq(described_class.new(data: { 'exception' => 'NoMethodError' }))
      end
    end
  ...
  describe '#hash' do
    ...
    context 'when an error and its parent class share to_h' do
      it 'returns different hash codes' do
        expect(error.hash).not_to eq(JSONRPC::Error.new('Internal JSON-RPC error.', code: -32_603).hash)
      end
    end
