...
RSpec.describe JSONRPC::InvalidParamsError do
  describe '#initialize' do
    ...
    context 'with a custom message' do
      let(:error) { described_class.new('Custom params error') }

      it 'uses the given message' do
        expect(error.message).to eq('Custom params error')
      end
    end

    context 'with optional data' do
      let(:error) { described_class.new(data: { 'errors' => ['param a is required'] }) }

      it 'sets data' do
        expect(error.data).to eq('errors' => ['param a is required'])
      end
    end
  ...
  describe '#to_h' do
    context 'without data' do
      ...
      it 'returns a hash with code and message' do
        expect(error.to_h).to eq(code: -32_602, message: 'Invalid method parameter(s).')
      end
    end
    ...
    context 'with data' do
      ...
      it 'includes data in the hash' do
        expect(error.to_h).to eq(
          ...
          data: { 'errors' => ['param a is required'] }
        )
      end
    end
  end
  ...
  describe '#to_json' do
    context 'without data' do
      ...
      it 'returns a JSON string with code and message' do
        expect(error.to_json).to eq('{"code":-32602,"message":"Invalid method parameter(s)."}')
      end
    end
  end
    ...
    context 'without request_id' do
      ...
      it 'returns a response hash with nil id' do
        expect(error.to_response).to eq(
          jsonrpc: '2.0',
          error: { code: -32_602, message: 'Invalid method parameter(s).' },
          id: nil
        )
      end
    end
  ...
  describe '#==' do
    ...
    context 'when the other object is a base Error with the same code and message' do
      it 'returns false' do
        expect(error).not_to eq(JSONRPC::Error.new('Invalid method parameter(s).', code: -32_602))
      end
    end

    context 'when the data differs' do
      it 'returns false' do
        expect(error).not_to eq(described_class.new(data: { 'errors' => ['param a is required'] }))
      end
    end
  ...
  describe '#hash' do
    ...
    context 'when an error and its parent class share to_h' do
      it 'returns different hash codes' do
        expect(error.hash).not_to eq(JSONRPC::Error.new('Invalid method parameter(s).', code: -32_602).hash)
      end
    end
